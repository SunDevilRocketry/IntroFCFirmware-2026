...
/**
  ...
  * @note This function must be called before using any other USB driver functions
  *       The UART handle should be initialized by CubeMX before calling this
  */
USB_STAT usb_init(UART_HandleTypeDef* huart, bool interruptEnabled);

// ==================== INTERRUPT-BASED FUNCTIONS ==================== //
/**
  * @brief Transmit data using interrupts (non-blocking with callback)
...
/**
  * @brief Receive data using interrupts (non-blocking with callback)
...
/**
  ...
  * @retval USB_STAT: USB_OK if successful, otherwise error code
  * @note Implementation using HAL_UART_Transmit directly
  */
...
/**
  ...
  * @retval USB_STAT: USB_OK if successful, otherwise error code
  * @note Implementation using HAL_UART_Receive directly
  */
